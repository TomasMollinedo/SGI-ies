...
/**
 * Seed de prueba SOLO para poder probar GET /cuentas-corrientes y
 * GET /cuentas-corrientes/:id/movimientos contra datos reales.
 *
 ...
 * Industrial Rivadavia" que crea `seed.ts`) y agrega encima 3 proveedores +
 * sus comprobantes/pagos.
 *
 ...
 * ComprobanteService.confirmar() y PagoService.create() si hoy tuvieran
 * controller (todavía no lo tienen). IVA simplificado a 0% (importe_neto =
 * importe_total) porque no aporta nada a lo que esto prueba.
 *
 * Es 100% idempotente (buscar-y-crear, nunca borra nada): correr
 *   npx tsx prisma/seed-cuenta-corriente-prueba.ts
 * las veces que haga falta no duplica datos.
 */
...
async function main() {
  ...
  console.log(
    'Seed de prueba - COMPROBANTEPROVEEDOR: 6 registros procesados.',
  );
  ...
  console.log('Seed de prueba - PAGO: 2 registros procesados.');
}
