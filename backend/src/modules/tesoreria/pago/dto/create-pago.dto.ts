...
/**
 * Una línea de imputación: cuánto de este pago se aplica a un comprobante
 * puntual. El importe lo propone la UI igual al saldo pendiente, pero acá
 * solo se fija la forma del contrato — que sea mayor a 0 y que no supere el
 * saldo pendiente del comprobante es una regla de negocio que depende de una
 * consulta a la base, y la valida el service.
 */
 ...
 * BORRADOR y admiten quedar vacíos hasta que se confirman, pero el pago nace
 * directamente CONFIRMADA (no tiene estado borrador), así que "al menos un
 * comprobante imputado" ya es exigible en el contrato de creación.
 *
...
export const createPagoSchema = z
  .object({
    ...
    detalle: z
      ...
      .min(1, 'El pago debe imputar al menos un comprobante'),
  })
  .refine(
    (data) =>
    ...
    {
      message:
        'No se puede imputar el mismo comprobante dos veces en el mismo pago',
      path: ['detalle'],
    },
  );

export class CreatePagoDto extends createZodDto(createPagoSchema) {}
